...
class ODriveNode(Node):
    def __init__(self):
        ...
        self.R_angle_publisher_ = self.create_publisher(Float32,'right_angle', 10)
        self.R_angle_publisher_timer = self.create_timer(0.1,self.R_angle_publisher_callback)
        
        self.L_angle_publisher_ = self.create_publisher(Float32,'left_angle', 10)
        self.L_angle_publisher_timer = self.create_timer(0.1,self.L_angle_publisher_callback)
        
        self.L_omega_publisher_ = self.create_publisher(Float32,'left_omega', 10)
        self.L_omega_publisher_timer = self.create_timer(0.1,self.L_omega_publisher_callback)

        self.R_omega_publisher_ = self.create_publisher(Float32,'right_omega', 10)
        self.R_omega_publisher_timer = self.create_timer(0.1,self.R_omega_publisher_callback)

        self.create_subscription(Float32, 'lwheel_vtarget', self.lwheel_vtarget_callback, 10)
        self.create_subscription(Float32, 'rwheel_vtarget', self.rwheel_vtarget_callback, 10)
    ...
    def request_state_callback(self, request: AxisState.Request, response: AxisState.Response):
        if self.is_driver_ready():
            if request.axis == 0:
                ...
                response.success = True
                response.state = self.driver.axis1.current_state
                response.message = f'Success'
            else:
    ...
    def R_angle_publisher_callback(self):
        if self.is_driver_ready():
            msg = Float32()
            msg.data = (self.driver.axis0.encoder.pos_estimate)
            self.R_angle_publisher_.publish(msg)
        else:
            self.get_logger().debug('ODrive not ready')
          
    def L_angle_publisher_callback(self):
        if self.is_driver_ready():
            msg = Float32()
            msg.data = -1* (self.driver.axis1.encoder.pos_estimate) #
            self.L_angle_publisher_.publish(msg)
        else:
            self.get_logger().debug('ODrive not ready')     

    def R_omega_publisher_callback(self):
        if self.is_driver_ready():
            msg = Float32()
            msg.data = (self.driver.axis0.encoder.vel_estimate)
            self.R_omega_publisher_.publish(msg)
        else:
            self.get_logger().debug('ODrive not ready')


    def L_omega_publisher_callback(self):
        if self.is_driver_ready():
            msg = Float32()
            msg.data = -1* (self.driver.axis1.encoder.vel_estimate) #
            self.L_omega_publisher_.publish(msg)
        else:
            self.get_logger().debug('ODrive not ready') 

    def lwheel_vtarget_callback(self, msg):
        if  self.is_driver_ready():
            self.driver.axis1.controller.input_vel =  msg.data #
        else:
            self.get_logger().debug('ODrive not ready')

    def rwheel_vtarget_callback(self, msg):
        if  self.is_driver_ready():
            self.driver.axis0.controller.input_vel = msg.data
        else:
            self.get_logger().debug('ODrive not ready')
